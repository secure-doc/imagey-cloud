...
Marys best friend is Laura (they are connected from the start). Alice is used in test scenarios to test establishing a new contact with Mary.

## Data Migration
As of the transition to a Document-based chat architecture:
- All test data (Pact mock contracts, unit tests) have been migrated so that Chats are represented as Documents of type `Chat` stored in a user's `Chat Folder`.
