    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) {
          throw new SecureDocError('Unauthorized', SecureDocErrorCode.UNAUTHORIZED);
        }
        if (response.status === 404) {
          throw new SecureDocError('Not found', SecureDocErrorCode.NOT_FOUND);
        }
        throw new SecureDocError(`HTTP Error: ${response.status}`, SecureDocErrorCode.NETWORK_ERROR);
      }
      return response;
    } catch (e) {
      if (e instanceof SecureDocError) throw e;
      throw new SecureDocError('Network request failed', SecureDocErrorCode.NETWORK_ERROR, e);
    }
  }
