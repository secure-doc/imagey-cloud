...
test("share a document in chat", async ({ page }) => {
  ...
  provider
    ...
    .withRequest(
      "GET",
      `/users/mary@imagey.cloud/documents/${documentId}/encrypted-shared-keys/mary@imagey.cloud`,
    )
    .willRespondWith(200, (r) =>
      r.jsonBody({
  ...
  provider
    ...
    .withRequest(
      "PUT",
      `/users/mary@imagey.cloud/documents/${documentId}/encrypted-shared-keys/laura@imagey.cloud`,
      (r) => {
        r.headers({ "Content-Type": "application/json" });
        r.jsonBody({
