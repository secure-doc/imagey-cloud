...
test("load existing profile with picture", async ({ page }) => {
  ...
  const profilePictureKeyInteraction = provider
    .addInteraction()
  ...
  const profilePictureInteraction = provider
    .addInteraction()
    ...
    .willRespondWith(200, (r) =>
      r.binaryFile(
        ...
        "./tests/images/encrypted/profile-pic-doc-id/contents/profile-pic-content-id",
      ),
    );

  const profileContentPath = path.join(
    process.cwd(),
    "tests",
    "images",
    "encrypted",
  ...
  fs.mkdirSync(path.dirname(profileContentPath), { recursive: true });
  fs.writeFileSync(
    profileContentPath,
    Buffer.from(profileMock.encryptedData, "base64"),
  );
  ...
  const profileContentInteraction = provider
    ...
    .willRespondWith(200, (r) =>
      r.binaryFile(
        "application/octet-stream",
        "./tests/images/encrypted/profile/mary@imagey.cloud/profile-content",
      ),
    );

  // When
  await profilePictureInteraction.executeTest(async (mockServer) => {
    ...
    const nameInput = page.getByLabel("Name");
    await expect(nameInput).toHaveValue("Mary Doe");
    ...
    const avatarImage = page.locator("article img").first();
