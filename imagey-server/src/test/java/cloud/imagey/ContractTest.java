...
public class ContractTest {
    ...
    @State("mary has open invitations")
    void maryHasOpenInvitations() throws URISyntaxException, IOException {
        marysSecondDeviceRegistered();
        File marysData = new File(rootPath, "mary@imagey.cloud");

        // Clear Marys documents to satisfy the empty documents stub
        File marysDocuments = new File(marysData, "documents");
        if (marysDocuments.exists()) {
            for (File file : marysDocuments.listFiles()) {
        ...
        // Add pending invitation from Alice
        File incomingInvitations = new File(new File(marysData, "invitations"), "incoming");
        File aliceInvite = new File(incomingInvitations, "alice@imagey.cloud");
        aliceInvite.mkdirs();
        org.apache.commons.io.FileUtils.write(
            new File(aliceInvite, "status.json"),
    ...
    void setTokenValid() throws URISyntaxException, IOException {
        tokenState = VALID_TOKEN;
    }
