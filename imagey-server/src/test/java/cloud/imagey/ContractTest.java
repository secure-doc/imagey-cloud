...
public class ContractTest {
    ...
    // Unlike the state above, Alice gets no direct-grant key here at all - the document exists in
    // Mary's tree, but Alice was never given a real witness for it, so a well-formed but
    // non-matching Access-Path chain (ADR 0009) is the *only* thing she can bring to the request.
    // Proves the header is actually parsed and verified server-side, not just trusted (see
    // DocumentRepository.verifyAccess / RolesFilter).
    @State("Alice has no access to Mary's uploaded document")
    void aliceHasNoAccessToMarysDocument() throws IOException, URISyntaxException {
        user = new User(new UserId(ALICE));
        maryHasUploadedDocument(); // ensures bb66aba3-8338-4ef4-a6f8-43ed0b39ecd3 exists
    }
