...
public class ContractTest {
    ...
    void maryHasNoContactsAndBillRequest() throws IOException {
        ...
        contactRequests.mkdirs();
        String json = "{\"invitee\":\"mary@imagey.cloud\",\"inviter\":\"bill@imagey.cloud\",\"status\":\"INVITED\","
            + "\"publicKey\":{\"crv\":\"P-256\",\"ext\":true,\"key_ops\":[],\"kty\":\"EC\","
            + "\"x\":\"nwGwyL6D7-mpGv3ahjdgFz7-FxEFSZZqWio5TvGEHWc\","
            + "\"y\":\"ubcX2RHk7odTGx6g7dgJpkhEBjzJ8YQ5q0wqtQc9Umc\"}}";
        java.nio.file.Files.writeString(new File(contactRequests, "bill@imagey.cloud.json").toPath(), json);
    }
    ...
    void maryHasChatWithAlice() throws IOException {
        ...
        new File(new File(getMarysData(), "contacts"), "alice@imagey.cloud").mkdirs();
        new File(new File(new File(rootPath, "alice@imagey.cloud"), "contacts"), "mary@imagey.cloud").mkdirs();
    }

    @State("Mary is logged in and has a chat with alice")
    void maryIsLoggedInAndHasChatWithAlice() throws IOException {
        user = getMary();
        maryHasChatWithAlice();
    }
    ...
    void maryHasChatWithLaura() throws IOException {
        ...
        new File(new File(getMarysData(), "contacts"), "laura@imagey.cloud").mkdirs();
        new File(new File(new File(rootPath, "laura@imagey.cloud"), "contacts"), "mary@imagey.cloud").mkdirs();
    }
    ...
    void maryHasChatWithBill() throws IOException {
        ...
        java.nio.file.Files.write(metadataEnc.toPath(), java.util.Base64.getDecoder().decode(
            "SvwyuvGK490PcZkvPvA3AiKeIehXCWjrEDomT57qgNdEVzRexUtOi7EpvekQPmQJSVIjB"
            + "p/3A6fUXLYIvlOElC326VhFbiA="));
        new File(new File(getMarysData(), "contacts"), "bill@imagey.cloud").mkdirs();
        new File(new File(new File(rootPath, "bill@imagey.cloud"), "contacts"), "mary@imagey.cloud").mkdirs();
    }
    ...
    void aliceHasChatWithMary() throws IOException {
        ...
        new File(new File(getAlicesData(), "contacts"), "mary@imagey.cloud").mkdirs();
        new File(new File(getMarysData(), "contacts"), "alice@imagey.cloud").mkdirs();
