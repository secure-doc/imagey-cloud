...
public class MessageRepository extends AbstractFileRepository {
    ...
    public Message persist(User receiver, User sender, String encryptedContent) throws IOException {
        ...
        write(messageFile, encryptedContent, UTF_8, false);
        return new Message(id, encryptedContent);
    }

    public List<Message> fetchMessages(User receiver, User sender, String sinceId) {
        File receiverHome = new File(rootPath, receiver.email().address());
        File messagesHome = new File(receiverHome, "messages");
        File senderFolder = new File(messagesHome, sender.email().address());
        ...
        if (senderFolder.exists()) {
            ...
            if (files != null) {
                ...
                for (File file : files) {
                    ...
                    if (sinceId == null || id.compareTo(sinceId) > 0) {
